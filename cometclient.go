...
func Cometclient(aaddr, key string, heartbeat int) {
	addr, err := net.ResolveTCPAddr("tcp", aaddr)
	if err != nil {
		log.Error("net.ResolveTCPAddr(\"tcp\", \"%s\") failed (%s)", aaddr, err.Error())
		return
	}
	...
	proto := []byte(fmt.Sprintf("*3\r\n$3\r\nsub\r\n$%d\r\n%s\r\n$%d\r\n%d\r\n", len(key), key, len(strconv.Itoa(int(heartbeat))), heartbeat))
